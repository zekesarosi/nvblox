...
void NvbloxNode::subscribeToTopics()
{
  ...
  // Settings for QoS.
  declare_parameter<std::string>("input_qos", kDefaultInputQos_);
  std::string input_qos_str = kDefaultInputQos_;
  get_parameter("input_qos", input_qos_str);
  rclcpp::QoS input_qos = rclcpp::QoS(10);
  if (input_qos_str == "SENSOR_DATA") {
    input_qos = rclcpp::SensorDataQoS();
  } else {
    input_qos = rclcpp::SystemDefaultsQoS();
  }
    ...
    for (int i = camera_index; i < params_.num_cameras; ++i) {
      ...
      depth_image_subs_.emplace_back(
        std::make_shared<::message_filters::Subscriber<sensor_msgs::msg::Image>>(
          this, base_name_depth + "/image", input_qos_profile));
      if (params_.use_segmentation) {
        ...
        segmentation_image_subs_.emplace_back(
          std::make_shared<::message_filters::Subscriber<sensor_msgs::msg::Image>>(
            this, base_name_seg_depth + "/image", input_qos_profile));
  ...
  if (params_.use_color) {
    for (int i = camera_index; i < params_.num_cameras; ++i) {
      ...
      color_image_subs_.emplace_back(
        std::make_shared<::message_filters::Subscriber<sensor_msgs::msg::Image>>(
          this, base_name_color + "/image", input_qos_profile));
      if (params_.use_segmentation) {
        ...
        segmentation_image_subs_.emplace_back(
          std::make_shared<::message_filters::Subscriber<sensor_msgs::msg::Image>>(
            this, base_name_seg_color + "/image", input_qos_profile));
        ...
        timesync_color_.emplace_back(
          std::make_shared<image_approx_sync>(
            image_approx_sync(kQueueSize), *color_image_subs_.back(),
            *color_camera_info_subs_.back()));
  ...
  const sensor_msgs::msg::CameraInfo::ConstSharedPtr & seg_camera_info)
{
  ...
  timing::Delays::tick(
    ...
    nvblox::Time(rclcpp::Time(depth_image->header.stamp).nanoseconds()),
  ...
  const sensor_msgs::msg::CameraInfo::ConstSharedPtr & seg_camera_info)
{
  ...
  timing::Delays::tick(
    ...
    nvblox::Time(rclcpp::Time(color_image->header.stamp).nanoseconds()),
    nvblox::Time(now().nanoseconds()));
...
void NvbloxNode::tick()
{
    ...
    shouldProcess(now, decay_tsdf_last_time_, params_.decay_tsdf_rate_hz))
  {
    if (isStaticOccupancy(params_.mapping_type)) {
      ...
      static_mapper_->decayOccupancyAllVoxels();
    } else {
      decayTsdf();
    }
  ...
  // 3D ESDF grid publish (decoupled rate from ESDF compute)
  ...
  return std::visit(
    Visitor{
      ...
      [this](const ImageMsgTuple & msg) -> bool {
        const auto & img_msg = std::get<kMsgTupleImageIdx>(msg);
        return this->canTransform(img_msg->header.frame_id, img_msg->header.stamp);
      },
      // Image + Mask
      [this](const ImageSegmentationMaskMsgTuple & msg) -> bool {
        const auto & img_msg = std::get<kMsgTupleImageIdx>(msg);
        const auto & mask_msg = std::get<kMsgTupleMaskIdx>(msg);
        return this->canTransform(img_msg->header.frame_id, img_msg->header.stamp) &&
               this->canTransform(mask_msg->header.frame_id, mask_msg->header.stamp);
      }
    }, variant_msg);
}
...
void NvbloxNode::decayTsdf()
{
  ...
  if (params_.use_depth) {
    ...
    static_mapper_->decayTsdfExcludeLastView<Lidar>();
  }
}
  ...
  const ImageTypeVariant & variant_msg)
{
  ...
  std::optional<ImagePtr> mask_img_opt;
...
bool NvbloxNode::processDepthImage(const ImageTypeVariant & depth_msg)
{
  ...
  if (mask_img_opt) {
    ...
    mask_image_timestamp = mask_img_opt.value()->header.stamp;
  }
  ...
  if (!conversions::depthImageFromRosMessageAsync(
      *depth_img_ptr, &depth_image_, &depth_image_tmp_, get_logger(),
      *cuda_stream_))
  {
  ...
  if (mask_img_opt) {
    if (!conversions::monoImageFromImageMessageAsync(
        *mask_img_opt.value(), &mask_image_,
        *cuda_stream_))
    {
...
bool NvbloxNode::processColorImage(const ImageTypeVariant & color_msg)
{
  ...
  if (!conversions::colorImageFromImageMessageAsync(
      *color_img_ptr, &color_image_, &rgb_image_tmp_, &bgra_image_tmp_, get_logger(),
      *cuda_stream_))
  {
